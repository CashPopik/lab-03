        ...
        if (showUpdateCityFields){
            Row(
                ...
                OutlinedTextField(
                    value = newProvinceName,
                    onValueChange = { newProvinceName = it },
                    label = { Text(selectedCity?.province ?: "") },
                    modifier = Modifier.weight(1f)
                )
                Spacer(modifier = Modifier.width(8.dp))
                Button(
                    ...
                    onClick = {
                        ...
                        if (newProvinceName.isBlank()){
                            newProvinceName = selectedCity?.province ?: ""
                        }
                        if (newCityName.isNotBlank() && newProvinceName.isNotBlank()) {
                            onUpdateCity(
                                selectedCity!!,
...
@Composable
fun CityRow(city: City, onClickCity: (City)->Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .padding(horizontal = 20.dp, vertical = 16.dp)
            .clickable(enabled = true, onClick = { onClickCity(city) })
    ) {
        Text(
            text = city.name,
            fontSize = 30.sp,
